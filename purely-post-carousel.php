...
Description: WordPress Carousel With Extended Options 
Version: 1.0.3
Author: PurelyThemes
...
class purely_post_carousel extends WP_Widget {
	...
	function purely_post_carousel() {
		parent::__construct(false, $name = __('post Carousel', 'purely_post_carousel') );
...
function custom_excerpt_length_post_carousel( $length ) {
	return 20;
}
add_filter( 'excerpt_length', 'custom_excerpt_length_post_carousel', 999 );
// Enqueue CSS FILES
...
// Enqueue JS Scripts in footer
function post_carousel_scripts() {
	wp_register_script('product_carousel_slick', plugins_url('slick.js', __FILE__), array('jquery'), 1.0, true);
	wp_enqueue_script('product_carousel_slick');
	wp_register_script('post_carousel_script', plugins_url('purely-post-carousel.js', __FILE__), array('jquery'), 1.0, true);
	wp_enqueue_script('post_carousel_script');
}
add_action( 'wp_enqueue_scripts', 'post_carousel_scripts', 20, 1 );